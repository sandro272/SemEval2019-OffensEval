...
from trial.config import TWEMOJI_LIST, LOGOGRAM, TWEMOJI, EMOTICONS_TOKEN
...
def main():
    ...
    for clf, name in [attentionModel(embeddingMatrix), capsulnetModel(embeddingMatrix), lstmModel(embeddingMatrix), gruModel(embeddingMatrix)]:
        train_set, test_set = get_stacking(clf, data, labels, testData, name=name)
    ...
    path = './pickle/stacking.pickle'
    pickle.dump([meta_train, meta_test, labels], open(path, 'wb'))
